...
jest.mock('./utils', () => ({
  GET_CONFIG_API: '/mock/config/api',
  DEFAULT_SELECTED_APP: 'desktop'
}));
...
describe('Configuration Component', () => {
      ...
      Promise.resolve({
        json: () =>
          Promise.resolve({
            apps: [{ name: 'desktop', has_ui: true, display_name: 'desktop' }],
            config: [
              { help: 'No help available.', key: 'desktop', is_anonymous: false, values: [] }
            ],
            conf_dir: '/conf/directory'
          })
      })
  ...
  describe('ConfigurationKey Component', () => {
    test('If the record has further values in it, should display record key, helpText as tooltip', () => {
      ...
      const tooltip = container.querySelector('.config-tooltip');
      expect(tooltip).toBeInTheDocument();
    });
  });

  test('If the record has no further values in it, renders record key, its value if available, additional help text with no tooltip, default value marked as "Default:" if one exists', () => {
    const record = {
      is_anonymous: false,
      key: 'Last Config Key',
      value: 'Some Value',
